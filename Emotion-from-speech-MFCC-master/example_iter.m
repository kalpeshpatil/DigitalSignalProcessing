...
%   Author: Kamil Wojcicki, September 2011
  ...
  %  clear all; close all; clc;  
    ...
    dir_path = '../../MeetDatabase/AudioData/AudioData/JK/';
...
%    
      ...
      t = MFCCs(:);
      fig = figure; 
      titl = char(strcat(file_name_array(k),'0',num2str(i)));
      sav = char(strcat(file_name_array(k),'0',num2str(i)));
    plot(t); 
    set(gca,'xtick',[]);
    title(['MFCC extracted feature ',titl]);axis off;
    
    saveas(fig,['../result/',sav,'.jpg'],'jpg');
    close(fig);
    
     [env_up,env_down] = envelope(1:size(t),t);
     
      fig = figure; 
      titl = char(strcat('env ',file_name_array(k),'0',num2str(i)));
      sav = char(strcat('env ',file_name_array(k),'0',num2str(i)));
      hold on;
    plot(env_up); 
    plot(env_down);
    set(gca,'xtick',[]);
    title(['MFCC extracted feature ',titl]);
    saveas(fig,['../result/env/',sav,'.jpg'],'jpg');
    disp(i);
     close(fig);
